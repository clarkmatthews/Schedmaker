...
The company name, default time zone, and the weekday the workweek starts on. New teams inherit these defaults. Each team can later use its own time zone and week start.
...
Create and rename teams, set each team’s time zone, week start, and color. Teams are the calendars in the menu.
